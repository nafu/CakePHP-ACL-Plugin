...
class AclManagerComponent extends Object {
	 ...
	 */
	public function getMissingAcos($update_hash_file = true) {
       ...
       */
			...
			if (!empty($updated_controllers)) {
				...
				foreach ($updated_controllers as $controller_name) {
					$controller_classname = $this->AclReflector->getControllerClassname($controller_name);

					$methods = $this->AclReflector->getControllerActions($controller_classname);
     ...
     */
       ...
       */
			$actions = $this->AclReflector->getControllerActions($controller_name);
			...
			if ($specific_permission_needed) {
				...
				if (!empty($aco_node)) {
					...
					if ($specific_permission !== false) {
						$delete_success = $this->Acl->Aro->Permission->delete(array(
								'Permission.id' => $specific_permission['Permission']['id']
							));
						//本来は、$delete_successの値で条件分岐させるべき
						//しかし、戻り値がnullになってしまうため
						//以下ではデータが存在するかどうかで条件分岐
						if (!$this->Acl->Aro->Permission->find('first', array(
									'conditions' => array(
										'aro_id' => $aro_id,
										'aco_id' => $aco_id
									)
