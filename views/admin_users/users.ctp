...
foreach($users as $user)
{
    ...
    foreach($roles as $role)
	{
	   ...
	   {
	       echo '  <td>' . $this->Html->image('/web/img/admin/acl/tick.png') . '</td>';
	   }
	   else
	   {
	   	   $title = __d('acl', 'Update the user role', true);
	       echo '  <td>' . $this->Html->link($this->Html->image('/web/img/admin/acl/tick_disabled.png'), '/admin/admin_users/update_user_role/?user=' . $user[$user_model_name][$user_pk_name] . '&role=' . $role[$role_model_name][$role_pk_name], array('title' => $title, 'alt' => $title, 'escape' => false)) . '</td>';
	   }
	}
	
    //echo '  <td>' . (isset($user['Aro']) ? $this->Html->image('/web/img/admin/acl/tick.png') : $this->Html->image('/web/img/admin/acl/cross.png')) . '</td>';
